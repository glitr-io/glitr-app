...
export default class Draggable extends Component {
	...
	constructor(props, defaultProps) {
		...
		this.panResponder = PanResponder.create({		
			...
			onPanResponderMove: Animated.event([null,{ 
				...
				listener: () => {
					onMove(this.state.pan.getLayout());
				}
			}),
			onPanResponderRelease: (e, gestureState) => {
				if(pressDragRelease)
					pressDragRelease(e, gestureState, this.state.pan.getLayout());
					this.reversePosition({
						x: this.state.pan.getLayout().left,
						y: this.state.pan.getLayout().top
					});
				if(reverse == false)
	...
	reversePosition = (newPos = {}) => {
		Animated.spring(						
			this.state.pan,				 
			{toValue:{x:0,y:0, ...newPos}}		 
		).start();
	}
	...
	render() {
		...
		return (
            <Animated.View 
                ...
                style={[this.state.pan.getLayout(), ...style]}>
                <TouchableOpacity 
